...
simulate_general <- function(
  ...
  alpha,        # Timescale of outcome variable response to effects
  ...
  Z_vec[1] <- Z_initial
  for(i in 1:(n_t - 1)) {
    Z_vec[i+1] <- exponential_decay(Z_vec[i], B_vec[i] + sum(X_mat[i,]), 1.0 / alpha, dt) + Z_noise[i]
  }
...
```{r}
result <- simulate_general(t_change_vec,  B_func, E_vec, T_mat, tau0_vec, tau1_vec, alpha, sigma_Z, dt)
```
...
```{r}
observe_outcome <- function(Z_func, t_obs_vec, sigma_obs) {
...
sigma_obs <- 4.0
Z_obs_vec <- observe_outcome(result$Z_func, t_obs_vec, sigma_obs)
...
Z_obs_vec_weird <- observe_outcome(result$Z_func, t_obs_vec_weird, sigma_obs)
...
sapply(c(0.1, 0.01, 0.001), function(dt) {
  system.time({
    simulate_general(t_change_vec,  B_func, E_vec, T_mat, tau0_vec, tau1_vec, alpha, sigma_Z, dt)
  })
})
```
