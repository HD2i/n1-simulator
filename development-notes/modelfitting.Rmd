...
We'll start by sourcing `n1simulator.R` for simulation functions:
...
```{r}
summ_nmle_noeffect <- summary(fit_nmle_noeffect)$tTable
print(summ_nmle_noeffect)
```

## Reusable functions

We want to return fit results in a standard format, a list containing:

* The raw fit object, because why not
* A data frame with treatment coefficients and p-values
* AIC value for the model

This function should do the trick for models 1-3:
...
And this one should work for model 4:
...
And one to allow you to choose a model by number:
