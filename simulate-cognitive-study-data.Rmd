...
For each of the 3 possible study lengths, we generate 3 datasets:
...
3. *B == A* : there is no difference between the two treatment effects
...
### Post-processing
Here are the parameter settings used to simulate data for the medium length (15 day) study. Note that `treatment_period=6` and `n_treatments=3`. Both of these settings will get modified in post-processing. 
```{r}
n_blocks = 2
n_treatments = 3
treatment_period = 6
